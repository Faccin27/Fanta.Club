    ...
    const fetchUpdates = async () => {
      try {
        const response = await fetch(
          "http://localhost:3535/announcements/buscaType?type=Updates"
        );
        const result: Updates[] = await response.json();
        setUpdate(result);
      } catch (err) {
        throw new Error(
          `Encontramos um erro para poder fazer o GET dos Updates. Verifique o erro: ${err}`
        );
      }
    };
    fetchUpdates();
            ...
            <div className="flex items-center mb-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Criado por:{" "}
                  <strong className="fonrt-bold text-orange-500">
                    {update.createdByName}
                  </strong>{" "}
                  no horário:{update.createdAt}
                </p>
              </div>
            </div>
            <Link href={`/forum/updates/${update.id}`}>
